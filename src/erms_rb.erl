...
-export([h/0, help/0, last/0, last/1, last/2]).
...
start(Options) ->
    supervisor:start_child(sasl_sup,
                           {erms_rb_server, {erms_rb, start_link, [Options]},
                            temporary, brutal_kill, worker, [erms_rb]}).

start_link(Options) ->
    gen_server:start_link({local, erms_rb_server}, erms_rb, Options, []).

stop() ->
    gen_server:call(erms_rb_server, stop),
    supervisor:delete_child(sasl_sup, erms_rb_server).

rescan() -> rescan([]).
rescan(Options) ->
    gen_server:call(erms_rb_server, {rescan, Options}, infinity).

list() -> ?MODULE:list(all).
list(Type) -> gen_server:call(erms_rb_server, {list, Type}, infinity).

show() ->
    gen_server:call(erms_rb_server, show, infinity).

show(Number) when is_integer(Number) ->
    gen_server:call(erms_rb_server, {show_number, Number}, infinity);
show(Type) when is_atom(Type) ->
    gen_server:call(erms_rb_server, {show_type, Type}, infinity).

grep(RegExp) -> gen_server:call(erms_rb_server, {grep, RegExp}, infinity).

start_log(FileName) -> gen_server:call(erms_rb_server, {start_log, FileName}).

stop_log() -> gen_server:call(erms_rb_server, stop_log).

h() -> help().
help() ->
    io:format("~nReport Browser Tool - usage~n"),
    io:format("===========================~n"),
    io:format("erms_rb:start()         - start the rb_server with default options~n"),
    io:format("erms_rb:start(Options)  - where Options is a list of:~n"),
    print_options(),
    io:format("erms_rb:h()             - print this help~n"),
    io:format("erms_rb:help()          - print this help~n"),
    io:format("erms_rb:list()          - list all reports~n"),
    io:format("erms_rb:list(Type)      - list all reports of type Type~n"),
    io:format("      currently supported types are:~n"),
    print_types(),
    io:format("erms_rb:grep(RegExp)    - print reports containing RegExp~n"),
    io:format("erms_rb:rescan()        - rescans the report directory with same~n"),
    io:format("                     options.~n"),
    io:format("erms_rb:rescan(Options) - rescans the report directory with new~n"),
    io:format("                     options. Options is same as in start/1.~n"),
    io:format("erms_rb:show(Number)    - print report no Number~n"),
    io:format("erms_rb:show(Type)      - print all reports of type Type~n"),
    io:format("erms_rb:show()          - print all reports~n"),
    io:format("erms_rb:start_log(File) - redirect all reports to file~n"),
    io:format("erms_rb:stop_log()      - close the log file and redirect to~n"),
    io:format("                     standard_io~n"),
    io:format("erms_rb:last()          - show the last 20 messages~n"),
    io:format("erms_rb:last(N)         - show the last N messages~n"),
    io:format("erms_rb:last(N,Types)   - show the last N messages of types Types~n"),
    io:format("erms_rb:stop            - stop the rb_server~n").
...
%%-----------------------------------------------------------------
%% Func: scan_files(Dir, Files, Max, Type, Leader)
...
print_typed_reports(Dir, Data, Type, Device, Abort, Log, Leader) ->
    {Next,Device1} =
        case element(2, hd(Data)) of
            Type ->
                print_report(Dir, Data, element(1, hd(Data)), Device, Abort, Log, Leader);
            _ ->
                {proceed,Device}
        end,
    if Next == abort ->
